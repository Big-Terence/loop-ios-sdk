...
open class LoopNotificationService: UNNotificationServiceExtension {
    ...
    private static let mediaSession: URLSession = {
        ...
        return URLSession(configuration: cfg, delegate: LoopMediaSessionDelegate(), delegateQueue: nil)
    }()
...
private final class LoopMediaSessionDelegate: NSObject, URLSessionDownloadDelegate {
    ...
    func urlSession(
        ...
        if totalBytesWritten > LoopNotificationService.maxMediaBytes
            || totalBytesExpectedToWrite > LoopNotificationService.maxMediaBytes {
            downloadTask.cancel()
        }
    }
