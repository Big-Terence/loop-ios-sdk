...
/// `LoopPush.register(provisional: true)` — no OS prompt is shown and a silent
/// token is granted immediately.
public struct LoopPrimerConfig: Sendable {
    ...
    /// When `true`, the Allow button calls `LoopPush.register(provisional: true)`
    /// (silent, no OS prompt). When `false` (default) it calls the standard
    /// `LoopPush.register()` which shows the OS prompt.
...
///
/// Present it with `.sheet(isPresented:) { LoopPrimerView(config: ...) }`.
...
/// `LoopPush.register(provisional:)` directly from your own button handler.
///
...
@available(iOS 14.0, *)
public struct LoopPrimerView: View {
    @Environment(\.dismiss) private var dismiss
    public let config: LoopPrimerConfig

    public init(config: LoopPrimerConfig = LoopPrimerConfig()) {
        self.config = config
    }
        ...
        VStack(spacing: 24) {
            ...
            VStack(spacing: 12) {
                Button(config.allowTitle) {
                    ...
                    Loop.track("push_primer", ["result": .string("allow"), "provisional": .bool(config.provisional)])
                    LoopPush.register(provisional: config.provisional)
                    dismiss()
                }
                ...
                Button(config.notNowTitle) {
                    Loop.track("push_primer", ["result": .string("dismiss"), "provisional": .bool(config.provisional)])
                    dismiss()
                }
                .font(.subheadline)
                .foregroundColor(.secondary)
