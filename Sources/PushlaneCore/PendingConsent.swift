...
///
/// Why this exists (the sacred invariant): Pushlane is opt-out by default, so the ONLY
...
enum PendingConsentStore {
    // legacy key kept across the Loop→Pushlane rebrand (device identity continuity) — do not change
