...
/// Push registration + open-tracking. Wire `PushlanePush.register()` and the
...
public enum PushlanePush {

    /// Category identifier attached (always-on) to every Pushlane push by the sender.
    ...
    // value kept across the Loop→Pushlane rebrand — wire contract with the APNs sender and shipped apps
    public static let categoryIdentifier = "LOOP_DEFAULT"

    #if canImport(UserNotifications)
    /// Register Pushlane's `LOOP_DEFAULT` category (with `.customDismissAction`) WITHOUT
    ...
    ///   Pushlane sends normally; the OS decides display style.
    public static func register(provisional: Bool = false) {
        ...
        center.requestAuthorization(options: options) { granted, _ in
            ...
            guard granted else {
                ...
                Pushlane.shared.reportAuthorizationLevel()
                return
            }
    ...
    public static func didRegister(deviceToken: Data) {
        ...
        UNUserNotificationCenter.current().getNotificationSettings { settings in
            ...
            Pushlane.shared.registerDeviceToken(deviceToken, authorizationLevel: level)
        }
        #else
        Pushlane.shared.registerDeviceToken(deviceToken, authorizationLevel: nil)
        #endif
    }
    ...
    public static func didFailToRegister(error: Error) {
        ...
        Pushlane.track("push_registration_failed", ["error": .string(String(describing: error))])
    }
}
...
final class OpenTracker: NSObject, UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(
        ...
        if response.actionIdentifier == UNNotificationDismissActionIdentifier {
            Pushlane.track("dismissed", props)
        } else {
